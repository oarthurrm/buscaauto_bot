...
# Configuração correta do ChatGroq
...
def consultar_carro(marca, modelo, ano):
    ...
    template = ChatPromptTemplate.from_messages([
        ('system', 'Você se chama Luigi. Você é um especialista em carros. Você conhece detalhes técnicos, vantagens e desvantagens de veículos. Se não souber, recomende procurar um mecânico. E retorne a resposta de forma organizada, com titulos em letras maiusculas e abaixo o detalhamento. Não coloque * na resposta e adicione espaçamento entre os titulos em cima e embaixo. '),
        ('user', '{input}')
    ])
    ...
    response = chain.invoke({'informations': document, 'input': input_user}).content  
